  ...
  .then(async res => {
    ...
    if(status == 'COMPLETED' || status == 'RUN_FAILED' ||status == 'RUN_ABORTED' || status =='RUN_TIMED_OUT' ||status == 'DEPLOY_FAILED' ||status == 'UNKNOWN'){
      if(status == 'COMPLETED'){
          ...
          io.to("Room").emit('message', formatMessage(servername, 'Your Request ID has been generated . Request ID:  ' + botOutput));
      }else{
...
app.post('/cc-botdeployment',(req,res)=>{
  ...
  //io.to("Room").emit('message', formatMessage(servername, 'Message from Bot : ' + JSON.stringify(req.body)));
  io.to("Room").emit('message', formatMessage(servername, 'Your Request ID has been generated . Request ID:  ' + (parseInt(req.body['request_id'])+1).toString()));
});
...
app.post('/cc-approval-status',(req,res)=>{
  ...
  //io.to("Room").emit('message', formatMessage(servername, 'Message from Bot : ' + JSON.stringify(req.body)));
  //io.to("Room").emit('message', formatMessage(servername, req.body['message']));
});
  ...
  socket.on('chatMessage', msg => {
    ...
    if(msg?.toLowerCase().includes("customer id")){
      ...
      authentication(input).then(status=>{
        io.to(user.room).emit('message', formatMessage(servername, user.username + " , your Credit Card request has been received and is being reviewed by our Manager shortly "));
      })
    }
